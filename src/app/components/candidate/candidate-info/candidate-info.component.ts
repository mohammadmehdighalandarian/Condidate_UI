...
export class CandidateInfoComponent implements OnInit {
  ...
  ngOnInit(): void {
 const id =
    history.state?.candidateGuid || sessionStorage.getItem('candidateGuid');
          if (!id) {
  ...
  async setTab(
    tab: 'profile' | 'biography' | 'social' | 'websites',
  ): Promise<void> {
    this.activeTab = tab;
  ...
  private async loadBasicInfo(): Promise<void> {
    ...
    try {
      const res: any = await this.candidateService.getCandidateBasicInformation(
        this.candidateId,
      );
      const data = res?.data || {};

      const base64Image = (data?.profileImage || '').replace(
        /(\r\n|\n|\r)/g,
        '',
      );
      ...
      this.candidateInfo = {
        ...this.candidateInfo,
        ...data,
        imageUrl: base64Image ? `data:image/jpeg;base64,${base64Image}` : '',
      };
  ...
  private async loadBiography(): Promise<void> {
    ...
    try {
      const res: any = await this.candidateService.getCandidateBiography(
        this.candidateId,
      );
  ...
  private async loadSocialMedia(): Promise<void> {
    ...
    try {
      const res: any = await this.candidateService.getCandidateSocialMedia(
        this.candidateId,
      );
  ...
  private async loadWebsites(): Promise<void> {
    ...
    try {
      const res: any = await this.candidateService.getCandidateWebsites(
        this.candidateId,
      );
